...
//echo $facultyList;
		...
		return view('page.portal.faculty.portal',  ['facultyCourseList' => $facultyCourseList],['facultySemList' => $facultySemList]);
		}
	else{
...
->get();
$StudentGrade = DB::table('t_course_grade')->get();
//echo $StudentGrade;
...
//echo $facultyStudentList;
 ...
 if(count($facultyStudentList) > 0)
				 {
				 	return view('page.portal.faculty.students', ['facultyStudentList' => $facultyStudentList],['StudentGrade' => $StudentGrade]);
				 }
				 else 
				 {
...
//studentListGet ends
	...
	/*//studentListGetGetbySem starts

public function studentListGetGetbySem($CourseId,Request $request){
    	if($request->session()->get('type') == 'faculty'){
...
//studentListGetGetbySem ends*/





	//gradeInsertoUpdate starts

public function gradeInsertoUpdate($c_faculty_id,Request $request){
    	if($request->session()->get('type') == 'faculty'){
...
$stdExist	= DB::table('t_course_grade')->where('g_courseId', $request->g_courseId)->where('g_student', $request->g_student)
->get();
...
// INSERToUpdate STARTS 
      // echo $req;

        if(count($stdExist) > 0)
        { //updade

        	  DB::table('t_course_grade')->where('g_courseId', $request->g_courseId)->where('g_student', $request->g_student)
->update([
	
		 
    'g_mid' => $request->g_mid,  
    'g_final' => $request->g_final
        ...
        {
        	//insert
        	  DB::table('t_course_grade')->insert([
    ['g_id' => $request->g_id,  
    'g_courseId' => $request->g_courseId,  
    'g_mid' => $request->g_mid,  
    'g_final' => $request->g_final ,
    'g_student' => $request->g_student 
...
$request->session()->flash('msg', "✓ GRADE PUBLUSHED !");
	//return redirect()->route('faculty.sectionDetails',$c_faculty_id);
	return back();
		}
	...
	//gradeInsertoUpdate  ends


// roomRequest starts


	 public function roomRequest(Request $req){
...
//insert statrs
       ...
       DB::table('t_room_request')->insert([
    [ 
    'r_number' => $req->r_number ,
    'r_message' => $req->r_message ,
    'r_date' => $req->r_date ,
    'r_faculty' => $req->session()->get('username')
...
//insert ends
     //  $msg="TSF Updated";
         //return view('page.registration.registration')->with('msg', 'complete');
       $req->session()->flash('msg', "✔ Your room request sent to register");
        		return back();
